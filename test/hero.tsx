  ...
  return (
    <section id="top" className="relative isolate min-h-screen overflow-hidden pb-24 pt-36">
        ...
        <div>
          ...
          <Reveal delay={90}>
            <h1 className="mt-8 text-balance text-5xl font-semibold leading-[0.98] md:text-7xl xl:text-[5.2rem]">
              The Future of{" "}
              <span className="text-gradient">AI Recruitment</span> Starts Here
            </h1>
          </Reveal>
          <Reveal delay={180}>
            <p className="mt-8 max-w-xl text-pretty text-base leading-relaxed text-muted-foreground md:text-lg">
              Empower organizations with intelligent hiring powered by machine learning,
              multilingual AI, predictive candidate matching, resume intelligence, ATS
              optimization, and GCC recruitment expertise.
            </p>
          </Reveal>
          <Reveal delay={260}>
            <div className="mt-10 flex flex-wrap items-center gap-4">
              <MagneticButton href="#cta">Get Started</MagneticButton>
          ...
          <Reveal delay={340}>
            <dl className="mt-14 grid max-w-lg grid-cols-3 gap-6 border-t border-border pt-8">
              {[
                ["2.4M+", "Talent profiles"],
                ["96%", "Match accuracy"],
                ["18 days", "Avg. time to hire"],
              ].map(([v, l]) => (
                <div key={l}>
      ...
      <div className="relative mx-auto mt-24 flex max-w-7xl items-center gap-3 px-6 text-[11px] uppercase tracking-[0.3em] text-muted-foreground">
        <Gauge className="h-4 w-4 text-primary" /> Scroll to explore the system
      </div>
    </section>
  );
}
