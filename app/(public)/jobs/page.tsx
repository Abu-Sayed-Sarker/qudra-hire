  ...
  return (
    <div className="min-h-screen flex flex-col bg-surface text-on-surface">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-8">
          ...
          <div className="flex flex-col sm:flex-row gap-3 animate-[fadeInUp_0.7s_0.3s_ease_forwards] opacity-0">
            <div className="relative flex-1">
              ...
              <input type="text"
                className="w-full bg-surface-deep border border-surface rounded-xl py-3 pl-11 pr-4 text-on-surface placeholder:text-on-surface-subtle focus:outline-none focus:border-[#4BC957]/50 transition-colors min-h-11"
                placeholder="Search roles, skills, companies..." />
            </div>
            <button className="bg-[#4BC957] hover:bg-[#00B96E] text-white font-bold px-6 sm:px-8 py-3 rounded-xl transition-all active:scale-[0.98] whitespace-nowrap min-h-11 text-sm">
              Search
            </button>
          </div>
        </div>
      ...
      <section className="flex-1 max-w-7xl mx-auto px-4 sm:px-8 py-6 md:py-10 w-full flex flex-col md:flex-row gap-6">
        ...
        {/* Sidebar */}
        <Animate className="animate-from-left w-full md:w-64 shrink-0 space-y-4 sm:space-y-6">
          ...
          {[
            ...
            <div key={title} className="bg-surface-card border border-surface rounded-2xl p-4 sm:p-5 shadow-sm">
              ...
              <div className="space-y-2.5 sm:space-y-3">
                {items.map(item => (
                  <label key={item} className="flex items-center gap-3 cursor-pointer group min-h-11">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-5">
            {jobs.map((job, idx) => (
              <Animate key={idx} className="animate-on-scroll" delay={delays[idx % 6]}>
                <div onClick={() => router.push("/login")} className="bg-surface-card border border-surface rounded-2xl p-5 sm:p-6 hover:border-surface transition-colors cursor-pointer group h-full min-h-11">
